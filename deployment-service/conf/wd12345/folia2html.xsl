...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:imdi="http://www.mpi.nl/IMDI/Schema/IMDI"
                xmlns:folia="http://ilk.uvt.nl/folia" xmlns:exsl="http://exslt.org/common"
      ...
      <head>
        ...
        <xsl:choose>
          ...
          <xsl:when test="folia:metadata//imdi:Session/imdi:Title">
            <title>
              <xsl:value-of select="folia:metadata[@type='imdi']//imdi:Session/imdi:Title"/>
            </title>
          </xsl:when>
          <xsl:when test="folia:metadata/folia:meta[@id='title']">
            <title>
              <xsl:value-of select="folia:metadata/folia:meta[@id='title']"/>
            </title>
          </xsl:when>
          <xsl:otherwise>
            <title>
  ...
  <xsl:template match="folia:w">
    ...
    <xsl:if
        test="not(ancestor::folia:original) and not(ancestor::folia:suggestion) and not(ancestor::folia:alternative) and not(ancestor-or-self::*/auth)">
      <span id="{@xml:id}">
        <xsl:attribute name="class">word
          <xsl:if test="//folia:wref[@id=$wid and not(ancestor::folia:altlayers)]">sh</xsl:if>
          <xsl:if test=".//folia:correction or .//folia:errordetection">cor</xsl:if>
        </xsl:attribute>
        <span class="t">
          <xsl:value-of
              select="string(.//folia:t[not(ancestor-or-self::*/@auth) and not(ancestor::folia:morpheme) and not(ancestor::folia:str) and not(@class)])"/>
        </span>
  ...
  <xsl:template match="folia:t">
    ...
    <xsl:if
        test="not(following-sibling::*//folia:t[(not(./@class) or ./@class='current') and not(ancestor-or-self::*/@auth) and not(ancestor::folia:suggestion) and not(ancestor::folia:alt) and not(ancestor::folia:altlayers) and not(ancestor::folia:morpheme) and not(ancestor::folia:str)])">
      <xsl:if
          test="(not(./@class) or ./@class='current') and not(ancestor-or-self::*/@auth) and not(ancestor::folia:suggestion) and not(ancestor::folia:alt) and not(ancestor::folia:altlayers) and not(ancestor::folia:morpheme) and not(ancestor::folia:str)">
        <xsl:apply-templates/>
      </xsl:if>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template name="tokenannotations">
    <span class="attributes">
      ...
      <xsl:if test=".//folia:lemma">
        <xsl:for-each
            select=".//folia:lemma[not(ancestor-or-self::*/@auth) and not(ancestor-or-self::*/folia:morpheme)]">
      ...
      <xsl:if test=".//folia:sense">
        <xsl:for-each
            select=".//folia:sense[not(ancestor-or-self::*/@auth) and not(ancestor-or-self::*/folia:morpheme)]">
      ...
      <xsl:if test=".//folia:metric">
        <xsl:for-each
            select=".//folia:metric[not(ancestor-or-self::*/@auth) and not(ancestor-or-self::*/folia:morpheme)]">
          <span class="attrlabel">Metric
            <xsl:value-of select="@class"/>
          </span>
          <span class="attrvalue">
      ...
      <xsl:if test=".//folia:errordetection">
        <xsl:for-each
            select=".//folia:errordetection[not(ancestor-or-self::*/@auth) and not(ancestor-or-self::*/folia:morpheme)]">
          ...
          <span class="attrvalue">
            <xsl:for-each select="folia:morpheme">
              <span class="morpheme">
                ...
                <xsl:if test="@class">
                  <span class="details">(<xsl:value-of select="@class"/>)
                  </span>
                </xsl:if>
                <xsl:if test="@function">
                  <span class="details">[function=<xsl:value-of select="@function"/>]
                  </span>
                </xsl:if>
                <xsl:if test=".//folia:pos">
                  <xsl:for-each select=".//folia:pos[not(ancestor-or-self::*/@auth)]">
                    <span class="details">[pos=<xsl:value-of select="@class"/>]
                    </span>
                  </xsl:for-each>
                </xsl:if>
                <xsl:if test=".//folia:lemma">
                  <xsl:for-each select=".//folia:lemma[not(ancestor-or-self::*/@auth)]">
                    <span class="details">[lemma=<xsl:value-of select="@class"/>]
                    </span>
                  </xsl:for-each>
                </xsl:if>
                <xsl:text> </xsl:text>
        ...
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:if test="//folia:w[@xml:id=$wrefid]">
              <xsl:value-of
                  select="//folia:w[@xml:id=$wrefid]/folia:t[not(ancestor::folia:original) and not(ancestor::folia:suggestion) and not(ancestor::folia:alternative) and not(ancestor-or-self::*/auth)]"/>
            </xsl:if>
            <xsl:text> </xsl:text>
          </xsl:otherwise>
        </xsl:choose>
  ...
  <xsl:template match="folia:su" mode="xml2layout">
    ...
    <xsl:variable name="childwidth">
      <xsl:value-of select="sum(exsl:node-set($subTree)/folia:su/@width)"/>
    </xsl:variable>

    <xsl:variable name="width">
      <xsl:choose>
        <xsl:when test="$childwidth > 1">
  ...
  <xsl:template name="layout2svg">
    ...
    <xsl:variable name="viewwidth">
      <xsl:value-of select="sum($layout/folia:su/@width) * 2 * $su.scale"/>
    </xsl:variable>
    <xsl:variable name="viewheight">
      <xsl:value-of select="$maxDepth * 2 * $su.scale"/>
    </xsl:variable>
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:variable name="rescale">
          <xsl:choose>
            <xsl:when test="$viewwidth &gt; 1024">
              <xsl:value-of select="1024 div $viewwidth"/>
            </xsl:when>
            <xsl:otherwise>1</xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
        ...
        <!-- Create SVG wrapper -->
        <svg viewbox="0 0 {$viewwidth}px {$viewheight}px" width="{$viewwidth}" height="{$viewheight + 25}px"
  ...
  <xsl:template match="folia:su" mode="layout2svg">
    ...
    <xsl:variable name="x"
                  select="(sum(preceding::folia:su[@depth = current()/@depth or (not(folia:su) and @depth &lt;= current()/@depth)]/@width) + (@width div 2)) * 2"/>
    ...
    <rect x="{($x - 0.9) * $su.scale}" y="{($y - 1) * $su.scale}" width="{1.8 * $su.scale}" height="{1 * $su.scale}"
          rx="{0.4 * $su.scale}" ry="{0.4 * $su.scale}"
    ...
    <text x="{$x  * $su.scale}" y="{($y + 0.3) * $su.scale}"
          style="text-anchor: middle; font-size: 9px; font-weight: normal;">
      <xsl:for-each select="folia:wref">
        <xsl:choose>
          <xsl:when test="@t">
            <xsl:value-of select="@t"/>
